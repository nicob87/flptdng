    ...
    try:
        if data_type in ["all", "messages"]:
            ...
            for msg in messages:
                ...
                print(
                    f"Raw message: {json.dumps(json.loads(msg['raw_message']), indent=2)}"
                )
                print("-" * 80)
        ...
        if data_type in ["all", "entries"]:
            ...
            for entry in entries:
                print(
                    f"{entry['timestamp']} | {entry['symbol']} | {entry['side']} | "
...
def main():
    parser = argparse.ArgumentParser(
        description="Query Kraken order book data from TimescaleDB"
    )
    ...
    print(
        f"Querying with: sort={args.sort}, count={args.count}, data_type={args.data_type}"
    )
