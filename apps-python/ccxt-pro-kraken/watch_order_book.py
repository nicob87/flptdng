...
class CustomKraken(ccxtpro.kraken):
    def __init__(self, config={}):
        ...
        if self.simulation_mode and self.replay_server_url:
            print(f"🎬 Simulation mode: Using replay server at {self.replay_server_url}")
...
async def main():
    ...
    parser.add_argument('-s', '--simulate', type=str, metavar='DATE',
                        help='Simulate/replay from date (YYYY-MM-DD or YYYY-MM-DDTHH:MM:SS)')
    parser.add_argument('--port', type=int, default=8080,
                        help='Replay server port (default: 8080)')
    ...
    if args.simulate:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(f"{server_url}/replay/prepare", 
                                      json={'date': args.simulate}) as resp:
                    if resp.status != 200:
        ...
        try:
            ...
            for i in range(20-2):
                try:
                    ...
                    print(f"🎬 SIMULATED orderbook {i}: {orderbook['symbol']} - Bids: {len(orderbook['bids'])}, Asks: {len(orderbook['asks'])}, BOOK: {orderbook}")
                    await sleep(1)
                except Exception as e:
    ...
    try:
        ...
        for i in range(20):
            orderbook = await exchange.watch_order_book('BTC/USD')
            await sleep(1)
            print(f"📈 Live orderbook update {i}: {orderbook['symbol']} - Bids: {len(orderbook['bids'])}, Asks: {len(orderbook['asks'])}")
    finally:
        try:
            await exchange.un_watch_order_book('BTC/USD')
            print("🛑 Un-watched order book")
